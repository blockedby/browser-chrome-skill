...
async function fixture(t) {
  ...
  await executable(
    path.join(root, 'scripts/open-headless.sh'),
  ...
  const env = {
    ...process.env,
    BROWSER_CHROME_NODE: process.execPath,
    OPENED: opened,
    CLOSED: closed,
    FIXTURE_STATE: path.join(base, 'state.json'),
  };
...
for (const mode of ['headed-connect', 'headed', 'headless']) {
  test(`${mode} preserves cwd and completes stdio MCP without npm`, { timeout: 10000 }, async (t) => {
    ...
    await writeFile(
      ...
      JSON.stringify({ dependencies: { broken: '^1.0.0' }, overrides: { broken: '1.0.0' } }),
    );
...
test('missing runtime fails before opening Chrome', async (t) => {
  ...
  await assert.rejects(
    exec('bash', [f.wrapper, 'headless'], { env: f.env }),
...
test('server failure preserves exit status and cleans up headless', async (t) => {
  const f = await fixture(t);
  await assert.rejects(
    exec('bash', [f.wrapper, 'headless'], { env: { ...f.env, FIXTURE_EXIT: '42' } }),
    (error) => error.code === 42,
  );
